...
<body class="bg-slate-50 font-sans antialiased text-slate-800 min-h-screen flex flex-col">
  ...
  <header class="bg-white border-b border-slate-200 h-20 px-6 flex items-center justify-between sticky top-0 z-40 shadow-xs shrink-0">
    ...
    <div class="flex items-center space-x-4">
      ...
      <div class="hidden md:flex items-center space-x-2 text-slate-500 font-mono text-xs font-semibold" title="Session Timeout">
       ...
       <div class="relative">
         <button id="profileDropdownBtn" onclick="toggleProfileDropdown(event)" class="flex items-center space-x-2.5 p-1.5 border border-transparent hover:border-slate-200/50 rounded-lg text-left select-none cursor-pointer focus:outline-none transition">
            <?php if (!empty($headerUser['profile_picture']) && $headerUser['profile_picture'] !== 'default-avatar.png'): ?>
              <img src="<?php echo htmlspecialchars($headerUser['profile_picture']); ?>" alt="User Avatar" class="h-8 w-8 rounded-lg object-cover border border-brand-border shrink-0">
            <?php else: ?>
           ...
           <div class="hidden sm:flex flex-col">
             <span class="text-xs font-bold text-slate-700 leading-none"><?php echo htmlspecialchars($headerUser['full_name']); ?></span>
         ...
         <!-- Dropdown Card -->
         <div id="profileDropdownMenu" class="hidden absolute right-0 mt-2 w-56 bg-white border border-slate-200 rounded-xl shadow-lg py-1 z-50 text-xs text-slate-600 transition-all transform scale-95 origin-top-right">
           <!-- User Header -->
           <div class="px-4 py-3 border-b border-slate-100 flex items-center gap-2.5">
             ...
             <div class="flex flex-col overflow-hidden">
               <span class="font-bold text-slate-800 truncate"><?php echo htmlspecialchars($headerUser['full_name']); ?></span>
               <span class="text-[9px] font-black text-slate-400 uppercase"><?php echo htmlspecialchars($headerUser['role']); ?></span>
             </div>
           </div>
           
           <!-- Dropdown Items -->
           <div class="py-1">
             <a href="<?php echo $basePath ?? '../'; ?>pages/profile/profile.php" class="flex items-center gap-2.5 px-4 py-2 hover:bg-slate-50 text-slate-700 transition font-medium">
               <i class="fa-solid fa-user-circle text-slate-400 text-[13px]"></i>
               <span>My Profile</span>
             </a>
             <a href="<?php echo $basePath ?? '../'; ?>pages/profile/settings.php" class="flex items-center gap-2.5 px-4 py-2 hover:bg-slate-50 text-slate-700 transition font-medium">
               <i class="fa-solid fa-gears text-slate-400 text-[13px]"></i>
               <span>System Settings</span>
             </a>
             <a href="<?php echo $basePath ?? '../'; ?>pages/profile/change-password.php" class="flex items-center gap-2.5 px-4 py-2 hover:bg-slate-50 text-slate-700 transition font-medium">
